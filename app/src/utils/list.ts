...
export const scrollToSection = (ref: SectionList, sectionTitle: string, list: SectionListProps<unknown>['sections'], animated = false) => {
    const sectionIndex = list.findIndex((section) => section.title === sectionTitle);
    ref.scrollToLocation({ sectionIndex, itemIndex: 0, viewPosition: 0, animated });
};
    ...
    (data: { data: readonly any[] }[] | null, index: number) => {
        ...
        switch (elementPointer.type) {
            case 'SECTION_HEADER':
                ...
                break;
            case 'ROW':
                length = getItemHeight(data[sectionIndex].data[elementPointer.index], sectionIndex, elementPointer.index);
                break;
            case 'SECTION_FOOTER':
