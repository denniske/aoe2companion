...
import PlayerList, { IPlayerListPlayer } from '@app/view/components/player-list';
                                            ...
                                            <MyText style={styles.players}>
                                                {filteredPlayers.map((p, i) => (
                                                    <MyText key={i} style={styles.players2}>
                                                        <MyText style={styles.player} onPress={() => gotoPlayer(p.profileId)}>
                                                            {formatPlayer(p, i)}
                                                        </MyText>
...
export function FeedAdd() {
    ...
    return <Search selectedUser={onSelect} action={(user: IProfilesResultProfile) => <FeedAction user={user} />} />;
}
...
export function FeedConfig() {
    ...
    const following = useSelector((state) => state.following);
    return <PlayerList list={following} selectedUser={onSelect} action={(user: IPlayerListPlayer) => <FeedAction user={user} />} />;
}
