...
const withWidgetSourceFiles = (config, { targetName, appGroupIdentifier }) => {
    return (0, config_plugins_1.withXcodeProject)(config, async (config) => {
        ...
        if (!fs.existsSync(widgetSourceDirPath)) {
            ...
            await fs.promises.copyFile(path.join(widgetStaticSourceDirPath, 'WidgetBundle.swift'), path.join(widgetSourceDirPath, 'WidgetBundle.swift'));
        ...
        proj.addSourceFile('WidgetBundle.swift', {
            target: targetUuid,
        }, groupUuid);
        return config;
    });
