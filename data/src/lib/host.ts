...
export type Host =
    ...
    | 'aoe2companion-data'
    | 'aoe2companion-socket'
    | 'aoe2companion-tournament';
...
export function getHost(host: Host) {
    ...
    const platformHost = Platform.OS === 'web' ? 'localhost' : Device.isDevice ? hostIP : Platform.select({ ios: 'localhost', android: '10.0.2.2' });
    ...
    switch (host) {
        ...
        case 'aoe2companion-tournament': {
            if (dev) {
                return `http://${platformHost}:3332/`;
            }
            return `https://tournament.${appConfig.hostAoeCompanion}/`;
        }
    }
}
